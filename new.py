...
if "responses" not in st.session_state:
    st.session_state.responses = []  # Store all responses for download
...
async def generate_responses(prompt, message_placeholder):
    ...
    while not st.session_state.stop_loop:
        ...
        st.session_state.responses.append(full_response)  # Save the response
...
with st.sidebar:
    ...
    if st.session_state.responses:
        with tempfile.NamedTemporaryFile(delete=False, suffix=".txt") as tmp_file:
            tmp_file.write("\n\n".join(st.session_state.responses).encode())
